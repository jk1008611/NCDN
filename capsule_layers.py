...
from keras.utils.conv_utils import conv_output_length, deconv_output_length#deconv_length
import numpy as np
...
#        print(" activations", activations.get_shape())
        activations = tf.reshape(activations,[1,activations.shape[1],activations.shape[2],activations.shape[3],activations.shape[4]])
        return activations
...
class DeconvCapsuleLayer(layers.Layer):
    ...
    def call(self, input_tensor, training=None):
        ...
        if self.upsamp_type == 'resize':
            ...
            # out_height = deconv_output_length(self.input_height, self.scaling, self.kernel_size, self.padding,None)
            # out_width = deconv_output_length(self.input_width, self.scaling, self.kernel_size, self.padding,None)
            out_height = deconv_output_length(input_length=self.input_height, filter_size=self.kernel_size, padding=self.padding, output_padding=None, stride=self.scaling)
            out_width = deconv_output_length(input_length=self.input_height, filter_size=self.kernel_size, padding=self.padding, output_padding=None, stride=self.scaling)
    ...
    def compute_output_shape(self, input_shape):
        ...
        output_shape[1] = deconv_output_length(output_shape[1], self.scaling, self.kernel_size, self.padding,None)
        output_shape[2] = deconv_output_length(output_shape[2], self.scaling, self.kernel_size, self.padding,None)
...
#    print("  votes_trans", votes_trans.get_shape())
...
def CapsuleBlock(x,seq):
    
     with tf.variable_scope("Capsule_1"+seq):
    
        x = tf.reshape(x,shape=[x.shape[0].value,x.shape[1].value,x.shape[2].value,1,x.shape[4].value])
        # print("x",x.get_shape())
        cap_1 = ConvCapsuleLayer(kernel_size=3, num_capsule=2, num_atoms=x.shape[-1].value, strides=2, padding='same',
                                             routings=1, name=seq+'_1')(x)
        # print("cap_1",cap_1.get_shape())
        cap_2 = ConvCapsuleLayer(kernel_size=3, num_capsule=1, num_atoms=x.shape[-1].value, strides=1, padding='same',
                                             routings=3, name=seq+'_2')(cap_1)
        # print("cap_2",cap_2.get_shape())
        cap_3 = DeconvCapsuleLayer(kernel_size=3, num_capsule=1, num_atoms=x.shape[-1].value, upsamp_type='deconv',
                                            scaling=2, padding='same', routings=3,
                                            name=seq+'_3')(cap_2)
        # print("cap_3",cap_3.get_shape())
        # print('=========')
        return cap_3
...
if __name__ == '__main__':
     ...
     conv1 = tf.get_variable("conv1",[1,80,80,1,32],initializer=tf.truncated_normal_initializer(stddev=0.01))
     
     with tf.Session() as sess:
         
         sess.run(tf.global_variables_initializer())
         
         cc = CapsuleBlock(conv1,"1")
         
         print(cc.get_shape())
         
           
#         primary_caps = ConvCapsuleLayer(kernel_size=5, num_capsule=2, num_atoms=32, strides=2, padding='same',
#                                         routings=1, name='primarycaps')(conv1)
#         print(primary_caps.get_shape())
#         
#         deconv_cap_1_1 = ConvCapsuleLayer(kernel_size=5, num_capsule=1,num_atoms=32, strides=1, padding='same',
#                                         routings=3, name='11')( primary_caps)
#         
#         print(deconv_cap_1_1.shape[1].value)
#         
